...
function ContactPage() {
  ...
  useEffect(() => {
    const fetchPageData = async () => {
      ...
      if (!key) {
        ...
        // Si c'est "contact" ou vide, utiliser "contact" par défaut
        if (!pathKey || pathKey === 'contact') {
          key = 'contact';
        } else {
          key = pathKey;
        }
      ...
      console.log('🔍 Recherche de la page contact avec clé:', key);
      ...
      if (error || !data) {
        console.error('❌ Erreur ou page contact non trouvée:', error);
        setError(true);
      } else {
        console.log('✅ Page contact trouvée:', data.title);
        setPageData(data);
      }
      setLoading(false);
    };
  ...
  // Rediriger vers la page par défaut si erreur
  useEffect(() => {
    ...
    return (
      <div style={styles.errorContainer}>
        ...
        <p style={{ marginTop: '10px', fontSize: '0.9rem', color: '#666' }}>
          Redirection vers la page contact dans quelques secondes...
        </p>
